	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		if (dependencies.get("entity") == null) {
			if (!dependencies.containsKey("entity"))
				CompleteMod.LOGGER.warn("Failed to load dependency entity for procedure Hammer!");
			return;
		}
		if (dependencies.get("x") == null) {
			if (!dependencies.containsKey("x"))
				CompleteMod.LOGGER.warn("Failed to load dependency x for procedure Hammer!");
			return;
		}
		if (dependencies.get("y") == null) {
			if (!dependencies.containsKey("y"))
				CompleteMod.LOGGER.warn("Failed to load dependency y for procedure Hammer!");
			return;
		}
		if (dependencies.get("z") == null) {
			if (!dependencies.containsKey("z"))
				CompleteMod.LOGGER.warn("Failed to load dependency z for procedure Hammer!");
			return;
		}
		if (dependencies.get("world") == null) {
			if (!dependencies.containsKey("world"))
				CompleteMod.LOGGER.warn("Failed to load dependency world for procedure Hammer!");
			return;
		}
		...
		if ((((entity.getCapability(CompleteModVariables.PLAYER_VARIABLES_CAPABILITY, null)
						.orElse(new CompleteModVariables.PlayerVariables())).hammerDir) == Direction.NORTH)) {
			sz = (double) -10;
		}
		else if ((((entity.getCapability(CompleteModVariables.PLAYER_VARIABLES_CAPABILITY, null)
						.orElse(new CompleteModVariables.PlayerVariables())).hammerDir) == Direction.SOUTH)) {
			sz = (double) 0;
		}
		else {
			sz = (double) -5;
		...
		for (int index0 = 0; index0 < (int) repeat; index0++) {
			if ((((entity.getCapability(CompleteModVariables.PLAYER_VARIABLES_CAPABILITY, null)
						.orElse(new CompleteModVariables.PlayerVariables())).hammerDir) == Direction.DOWN)) {
			sy = (double) -10;
			}
			else if ((((entity.getCapability(CompleteModVariables.PLAYER_VARIABLES_CAPABILITY, null)
						.orElse(new CompleteModVariables.PlayerVariables())).hammerDir) == Direction.UP)) {
			sy = (double) 0;
			}
			else {
			sy = (double) -1;
			...
			for (int index1 = 0; index1 < (int) repeat; index1++) {
				if ((((entity.getCapability(CompleteModVariables.PLAYER_VARIABLES_CAPABILITY, null)
						.orElse(new CompleteModVariables.PlayerVariables())).hammerDir) == Direction.EAST)) {
				sx = (double) 0;
				}
				else if ((((entity.getCapability(CompleteModVariables.PLAYER_VARIABLES_CAPABILITY, null)
						.orElse(new CompleteModVariables.PlayerVariables())).hammerDir) == Direction.WEST)) {
				sx = (double) -10;
				}
				else {
				sx = (double) -5;
		...
		if (world instanceof ServerWorld) {
			((World) world).getServer().getCommandManager().handleCommand(new CommandSource(ICommandSource.DUMMY, new Vector3d(x, y, z),
					...
					"/summon item ~ ~ ~ {Item:{id:\"complete:black_hole\",Count:1b,tag:{" + Test + "}}}");
		}
		ItemNumbers.clear();
		Items.clear();
	}
