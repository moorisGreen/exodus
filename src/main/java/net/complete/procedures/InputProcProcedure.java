	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		if (dependencies.get("world") == null) {
			if (!dependencies.containsKey("world"))
				CompleteMod.LOGGER.warn("Failed to load dependency world for procedure InputProc!");
			return;
		}
		if (dependencies.get("x") == null) {
			if (!dependencies.containsKey("x"))
				CompleteMod.LOGGER.warn("Failed to load dependency x for procedure InputProc!");
			return;
		}
		if (dependencies.get("y") == null) {
			if (!dependencies.containsKey("y"))
				CompleteMod.LOGGER.warn("Failed to load dependency y for procedure InputProc!");
			return;
		}
		if (dependencies.get("z") == null) {
			if (!dependencies.containsKey("z"))
				CompleteMod.LOGGER.warn("Failed to load dependency z for procedure InputProc!");
			return;
		}
		...
		if (id != 0) {
			...
			if (global.contains(id + "(")) {
				...
				if (global.contains(id + "(" + amount + ")" + nbt)) {
					CompleteModVariables.MapVariables.get(world).items = global.replace(id2 + amount + ")" + nbt, id + "(" + newnum + ")" + nbt);
				}
				else {
					CompleteModVariables.MapVariables.get(world).items = (global + id + numstr + nbt);
				}
			}
			else {
				CompleteModVariables.MapVariables.get(world).items = (global + id + numstr + nbt);
