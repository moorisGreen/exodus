...
public class OutputProcProcedure {
	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		if (dependencies.get("world") == null) {
			if (!dependencies.containsKey("world"))
				ExodusMod.LOGGER.warn("Failed to load dependency world for procedure OutputProc!");
			return;
		}
		if (dependencies.get("x") == null) {
			if (!dependencies.containsKey("x"))
				ExodusMod.LOGGER.warn("Failed to load dependency x for procedure OutputProc!");
			return;
		}
		if (dependencies.get("y") == null) {
			if (!dependencies.containsKey("y"))
				ExodusMod.LOGGER.warn("Failed to load dependency y for procedure OutputProc!");
			return;
		}
		if (dependencies.get("z") == null) {
			if (!dependencies.containsKey("z"))
				ExodusMod.LOGGER.warn("Failed to load dependency z for procedure OutputProc!");
			return;
		}
	...
	private static void ChangeItem(int free, int add, String global, String id, BlockPos pos, int slot, IWorld world, int max, String nbt) {
		...
		if (Integer.parseInt(number) <= free) {
			...
			ExodusModVariables.MapVariables.get(world).items = global.replace(id + "(" + number + ")" + nbt, "");
		}
		else {
			TileEntity _ent = world.getTileEntity(pos);
			if (_ent != null) {
			...
			ExodusModVariables.MapVariables.get(world).items = global.replace(id + "(" + number + ")" + nbt, id + "(" + newnumber + ")" + nbt);
		}
	}
}
